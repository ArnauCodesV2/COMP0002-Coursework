...
fromList = foldr Coursework.insert Leaf
...
insert x (Node v l r) = case compare x v of
  LT -> Node v (Coursework.insert x l) r
  GT -> Node v l (Coursework.insert x r)
  EQ -> Node v l r

-- join two Sets together be careful not to introduce duplicates.
union :: (Ord a) => Set a -> Set a -> Set a
union = foldr Coursework.insert
...
member _ Leaf = False -- Leaf represents the empty set
member x (Node v l r) 
    | x < v = member x l
    | x > v = member x r -- Literally definiton of traversal order
    | x == v = True

-- how many elements are there in the Set?
cardinality :: Set a -> Int
cardinality = foldr (\x acc -> acc + 1) 0

-- apply a function to every element in the Set
setmap :: (Ord b) => (a -> b) -> Set a -> Set b
setmap _ Leaf = Leaf
setmap f (Node v l r) = Node (f v) (setmap f l) (setmap f r)
